...
const UserProfilePageComponent = ({updateUserApiRequest, fetchUser, userInfoFromRedux, setReduxUserState, reduxDispatch, localStorage, sessionStorage}) => {
    ...
    useEffect(() => {
        fetchUser(userInfo._id).then((data) => setUser(data)).catch((error) => console.log(error))
    }, [userInfo._id])
    const onChange = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (event.currentTarget.checkValidity() === true && form.password.value === form.confirmPassword.value) {
            updateUserApiRequest(name, lastName, phoneNumber, address, country, zipCode, city, state, password).then(data => {
                ...
                reduxDispatch(setReduxUserState({doNotLogout: user.doNotLogout, ...data.userUpdated}))
            ...
            }).catch(error => setUpdateUserResponseState({error: error.response.data.message ? error.response.data.message : error.response.data}))
        }
        setValidated(true);
    };
    return (
                ...
                <Col md={6}>
                    ...
                    <Form noValidate validated={validated} onSubmit={handleSubmit}>
                        ...
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            ...
                            <Form.Control
                                disabled
                                value={user.email + " if you want to change email, remove account and create a new one with new email address"}
                            />
                        </Form.Group>
                        ...
                        <Alert show={updateUserResponseState && updateUserResponseState.error !== ""} variant="danger">Something went wrong</Alert>
                        <Alert show={updateUserResponseState && updateUserResponseState.success === "user updated"} variant="info">User updated</Alert>
                    </Form>
                </Col>
            </Row>
        </Container>
