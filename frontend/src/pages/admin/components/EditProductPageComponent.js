...
const EditProductPageComponent = ({categories, fetchProduct, updateProductApiRequest, reduxDispatch, saveAttributeToCatDoc, imageDeleteHandler, uploadImagesApiRequest, uploadImagesCloudinaryApiRequest}) => {
    const [validated, setValidated] = useState(false);
    const [product, setProduct] = useState({})
    const [updateProductResponseState, setUpdateProductResponseState] = useState({message: "", error: ""})
    const [attributesFromDb, setAttributeFromDb] = useState([])
    ...
    const {id} = useParams()
    useEffect(() => {
        fetchProduct(id).then((product) =>setProduct(product)).catch((error) => console.log(error))
    ...
    const handleSubmit = (event) => {
        ...
        if (event.currentTarget.checkValidity() === true) {
            ...
            }).catch((error) => setUpdateProductResponseState({error: error.response.data.message ? error.response.data.message : error.response.data}))
        }
        setValidated(true);
    };
    useEffect(() => {
                ...
                <Col md={6}>
                    ...
                    <Form noValidate validated={validated} onSubmit={handleSubmit} onKeyDown={(e) => checkKeyDown(e)}>
                        ...
                        <Form.Group className="mb-3" controlId="formBasicCategory">
                            ...
                            <Form.Select required name="category" aria-label="Default select example" onChange={changeCategory}>
                            ...
                            <Row className="mt-5">
                                <Col md={6}>
                                    <Form.Group className="mb-3" controlId="formBasicAttributes">
                                        ...
                                        <Form.Select name="attrKey" aria-label="Default select example" ref={attrKey} onChange={setValuesForAttrFromDbSelectForm}>
                                ...
                                <Col md={6}>
                                    <Form.Group className="mb-3" controlId="formBasicAttributes">
                                        ...
                                        <Form.Select name="attrKey" aria-label="Default select example" ref={attrVal} onChange={attributeValueSelected}>
                                            <option>Choose an attribute</option>
                                        </Form.Select>
                                    </Form.Group>
                                </Col>
                        ...
                        <Row>
                            <Col md={6}>
                                <Form.Group className="mb-3" controlId="formBasicNewAttribute">
                                    ...
                                    <Form.Control ref={createNewAttrKey} disabled={categoryChoosen === "Choose category"} placeholder="first choose or create category" name="newAttrKey" type="text" onKeyUp={newAttrKeyHandler} required={newAttrValue}/>
                                </Form.Group>
                            </Col>
                            <Col md={6}>
                            <Form.Group className="mb-3" controlId="formBasicNewAttribute">
                                    <Form.Label>Attribute value</Form.Label>
                                    <Form.Control ref={createNewAttrVal} disabled={categoryChoosen === "Choose category"} placeholder="first choose or create category" name="newAttrValue" type="text" onKeyUp={newAttrValueHandler} required={newAttrKey}/>
                                </Form.Group>
                            </Col>
                        </Row>
                        <Alert show={newAttrKey && newAttrValue} variant="primary">After typing attribute key and value press enter on one of the fields</Alert>
                        <Form.Group controlId="formFileMultiple" className="mb-3 mt-3">
                            ...
                            <Form.Control required type="file" multiple onChange={e => {
                                ...
                                if (process.env.NODE_ENV === "product") {
                                    ...
                                    }).catch((error) => setIsUploading(error.response.data.message ? error.response.data.message : error.response.data))
                                } else {
